...
PROJECT = "Voxgig Create SDK Generator"

# The vocabulary directory under .vale/styles/config/vocabularies/.
VOCAB = "CreateSdkgen"

# The root pages, always required. The README is the only reader-facing
# page here: the tutorial, how-to, reference and explanation pages live
# in the sdkgen repository, which the README links to.
ROOT_PAGES = ("README.md",)

# The documentation tree. None: this repository has no docs/ directory.
DOC_GLOBS = ()
NOT_DOCS = ()

# Pages outside the tree. None. CONTRIBUTING.md is a contributor working
# document, and everything under project/ is scaffold copied into a
# generated SDK project, where sdkgen's own documentation rules apply.
EXTRA_PAGES = ()

# Multi-port layout (one directory per language, each carrying its own
# pages). Off here: there is one package, at the repository root.
PORT_DIRS = False
NOT_PORTS = ()
PORT_PAGES = ()

# Where "we" is allowed: the tutorial, which walks through code with the
# reader (voice rule 7). There is no tutorial page in this repository.
TUTORIAL_PAGES = ()
...
# working document". CONTRIBUTING.md is deliberately absent: it is a
# contributor guide the README may point to.
WORKING_DOCS = (
